...
Signal 1 was implemented literally per spec first (a hand-built perplexity proxy), but proved fundamentally objective under real testing — no amount of recalibration against the toy reference corpus produced a signal that reliably separated known-AI from known-human text. It either collapsed to a constant or saturated at the extremes. Rather than keep patching an approach that was overfitting to a handful of test sentences, Signal 1 was replaced with a Groq-hosted LLM classifier. This is a deliberate, documented deviation, kept because it satisfies the milestone's actual acceptance bar such as meaningful score variation, correct classification on known cases, where the spec's literal design did not.

## AI Usage

I use AI companion for these three sections:

1. For Signal 1 (perplexity). I directed the assistant to implement expectedness_score per the spec's perplexity-based description. AI produced a placeholder reference LM, clearly flagged as a stand-in. After testing against 4 deliberately-chosen inputs from instructions showed it was objective — scores clustered near-constant regardless of actual content. Overrode by reverting expectedness_score to an honest always-unreliable stub rather than continuing to hand-tune a fundamentally limited approach, and built a working Signal 1 a different way (see below).
2. For Signal 1 (LLM classifier). I give the assistant an instruction to build a Groq-based attribution function with a simple prompt just asking for a verdict. Through another testing against the same 4 inputs revealed a systematic bias: every single input, including a clearly-AI paragraph, came back classified human with 90%+ (0.9) confidence. Then I overrode by rewriting the prompt to list concrete structural AI-vs-human criteria and switching the model from llama-3.1-8b-instant to the larger llama-3.3-70b-versatile. After re-testing against the same 4 inputs it showed me that the bias was gone —  out of 4, 3 are now matched intuition, with the 4th being the documented formal-writing edge case above, considered as AI-generated which is not a bug.
3. For Transparency label wording. I directed the assistant to implement generate_label() word-for-word from the planning doc's Transparency Label Design section. In return, system flagged my output and showed me that the spec's literal text contained typos, a duplicated sentence in one variant, and a genuine numeric inconsistency (the Thresholds table said the AI bucket starts at 0.66; the Transparency section said 0.65). After that I overrode the spec's literal (buggy) text: I corrected the wording and standardized on 0.65 as the single threshold used everywhere in the code, rather than silently picking one value or reproducing known errors verbatim.
